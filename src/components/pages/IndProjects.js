                   ...
                   <div className="IndProject">
                    ...
                    {(e.Sections) &&
                      e.Sections.map((Sections,id) =>
                        ...
                        {(Sections.header) && <h3 className="sectionHeader"> {Sections.header} </h3>}
                        {(Sections.img) && <img className="imgfull" src={Sections.img} />}
                        <h2>
                          {(Sections.words) && <span> {Sections.words} </span>}
                          {(Sections.title) && <span> {Sections.title}</span>}
                          {(Sections.offsite) && <a className="link" target="_blank" href={Sections.offsite}>{Sections.offsitewords}<Awayarrow className="arrow"/></a>}
