      ...
      {ProjectList.filter(e => e.link === location.pathname).map((e,id) =>
                   <div className="IndProject">
                    ...
                    {(e.Sections) &&
                      e.Sections.map((Sections,id) =>
                        ...
                        {(Sections.img) && <div className="imgcontain"> <h3 className="caption">↓ {Sections.caption}</h3> <img className="imgfull" src={Sections.img} /></div>}
                        <h2>
          ...
          <h3 className="footer">
            {ProjectList.map((e,id) =>
              <span className="pagelink"><NavLink to={e.link}> {id+1}.{e.title}</NavLink></span>
            )}
          </h3>
      </div>
    );
